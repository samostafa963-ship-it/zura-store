...
export default function RegisterPage() {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.email || !form.password) return setError('يرجى ملء جميع الحقول');
    ...
    <div style={{minHeight:'100vh',background:'#f5f5f5',display:'flex',alignItems:'center',justifyContent:'center',direction:'rtl',padding:20}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:0,background:'#fff',borderRadius:20,overflow:'hidden',boxShadow:'0 20px 60px rgba(233,30,140,.1)',maxWidth:900,width:'100%'}}>
        ...
        <div style={{padding:40,display:'flex',flexDirection:'column',justifyContent:'center'}}>
          ...
          <h2 style={{fontSize:22,fontWeight:900,marginBottom:4}}>إنشاء حساب جديد</h2>
          <p style={{fontSize:13,color:'#888',marginBottom:24}}>سجل الآن لتجربة تسوق سهلة وسريعة</p>

          {/* Google */}
          <button onClick={() => signIn('google', { callbackUrl: '/' })}
            style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10,background:'#fff',color:'#1a1a1a',border:'2px solid #f0e0ee',padding:'12px 20px',borderRadius:10,fontSize:14,fontWeight:700,cursor:'pointer',fontFamily:'inherit',marginBottom:16}}
          ...
          <div style={{textAlign:'center',fontSize:12,color:'#bbb',margin:'4px 0 16px'}}>أو بالبريد الإلكتروني</div>

          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {[
              {name:'name', placeholder:'الاسم الكامل', icon:'👤', type:'text'},
              {name:'email', placeholder:'البريد الإلكتروني', icon:'✉️', type:'email'},
              {name:'password', placeholder:'كلمة المرور', icon:'🔒', type:'password'},
              {name:'confirm', placeholder:'تأكيد كلمة المرور', icon:'🔒', type:'password'},
            ].map(f => (
              <div key={f.name} style={{display:'flex',alignItems:'center',background:'#f8f0f6',borderRadius:10,border:'1.5px solid #f0e0ee',padding:'0 14px'}}>
                <span style={{color:'#ccc',fontSize:15}}>{f.icon}</span>
          ...
          <div style={{textAlign:'center',fontSize:13,color:'#888',marginTop:16}}>
            لديك حساب بالفعل؟{' '}
            <a href="/login" style={{color:'#E91E8C',fontWeight:700,textDecoration:'none'}}>تسجيل الدخول</a>
          </div>
        </div>
        ...
        <div style={{background:'linear-gradient(135deg,#E91E8C,#c2185b)',padding:40,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center'}}>
          ...
          <h3 style={{color:'#fff',fontSize:22,fontWeight:900,marginBottom:12}}>انضم لزورا</h3>
          <p style={{color:'rgba(255,255,255,.8)',fontSize:14,lineHeight:1.7,marginBottom:24}}>وتمتع بأفضل تجربة تسوق<br/>مع آلاف المنتجات</p>
